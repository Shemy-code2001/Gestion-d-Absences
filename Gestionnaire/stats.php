<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['grp'])) {
        ...
        try {
            ...
            $req=$conn->prepare("SELECT s.nom,s.prenom,s.groupe, ab.cumul*2.5 as cumul,ab.cumul_nj*2.5 as nonjustif     
                            FROM stagiaire s
                            RIGHT JOIN(
                            SELECT a.idStg,abt.cumul,abnj.cumul_nj
                            FROM
    ...
    function getStartAndEndDateM($month, $year) {
        $debut = new DateTime("$year-$month-01");
        $fin = clone $debut;
        $fin->modify('last day of this month');
        return array($debut->format('Y-m-d'), $fin->format('Y-m-d'));
    }
    if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['week']) || isset($_POST['month']))) {
        extract($_POST);
        $grp = ["DEV101","DEV102","DEV103","INFO101","INFO102","ID101","ID102","ID103"];
        if (isset($week)&& !empty($week)) {
            ...
            $mois = substr($month, 5, 2);
            list($debut, $fin) = getStartAndEndDateM($mois, $annee);
        }
        else{
            $err = "Please provide either a week or a month.";
        }
        if(empty($err)){
            $ismo_absences = [];
            foreach($grp as $g){
                try {
                    $stmt = $conn->prepare("
                        SELECT s.nom, s.prenom, s.groupe, abt.cumul*2.5 AS cumul
                        FROM stagiaire s
                        RIGHT JOIN(SELECT idStg,count(*) as cumul FROM absence WHERE statut= 'traité' GROUP BY idStg) as abt USING(idStg)
                        RIGHT JOIN absence a USING(idStg)
                        LEFT JOIN seance sc USING(idSeance)
                        ...
                        ORDER BY s.groupe
                    ");
                    $stmt->execute([$g,$debut, $fin]);
                    $absences = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    if (!empty($absences)) {
                        $ismo_absences[$g] = $absences;
                    }
                } catch (PDOException $e) {
                    echo "Error: " . $e->getMessage();
                }
...
<header>
    ...
    <nav>
        ...
        <div class="attendance-form">
            <div class="content">
                <div class="tabs">
                    <button class="tab-btn active" onclick="showTab('SEMAINE')">Par Semaine</button>
                    <button class="tab-btn" onclick="showTab('MOIS')">Par Mois</button>
                </div>
                <form action="" method="POST" onsubmit="setActiveTab('SEMAINE')">
                    <div id="SEMAINE" class="tab-content">
                    <?php if(isset($err)) echo "<h3>$err</h3>" ;?>
                        <input type="week" class="week" name="week">
                        <input type="submit" name="val" style="margin-bottom: 20px; padding: 10px 20px; background-color: #27ae60; color: white; border: none; border-radius: 5px; cursor: pointer;" value="Valider">
                    </div>
                </form>

                <form action="" method="POST" onsubmit="setActiveTab('MOIS')">
                    <div id="MOIS" class="tab-content" style="display:none;">
                    <?php if(isset($err)) echo "<h3>$err</h3>" ;?>
                        <input type="month" class="month" name="month">
                        <input type="submit" name="val" style="margin-bottom: 20px; padding: 10px 20px; background-color: #27ae60; color: white; border: none; border-radius: 5px; cursor: pointer;" value="Valider">
                    </div>

                </form>
                <?php
                    if (isset($ismo_absences) && !empty($ismo_absences)) {
                        echo "Liste des stagiaires absents :";
                        foreach ($ismo_absences as $key => $ismo_absence) {
                            if(!empty($ismo_absence)){
                                echo "<br><h2>$key</h2>";
                                echo "<table border='1'>";
                                echo "<tr><th>Nom</th><th>Prénom</th><th>Groupe</th><th>Cumul</th></tr>";
                                foreach ($ismo_absence as $absence) {
                                    echo "<tr>";
                                    ...
                                    echo "<td>" . htmlspecialchars($absence['cumul']) . "</td>";
                                    echo "</form>";
                                    echo "</tr>";
                                }
                                echo "</table>";
